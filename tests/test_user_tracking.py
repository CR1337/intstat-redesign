...
def test_user_tracking(database_connection, table_definition):
    ...
    result = fetch(
        connection,
        f"SELECT ersteller_nutzer_id FROM view_{table_name}_aktuell WHERE {table_name}_id = {new_id};",
    )
    ...
    result = fetch(
        connection,
        f"SELECT nutzer_id FROM view_nutzer_aktuell WHERE nutzer_id = {ersteller_nutzer_id};",
    )
