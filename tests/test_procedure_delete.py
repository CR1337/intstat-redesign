...
def test_procedure_delete(database_connection, table_definition):
    ...
    if table_name == USER_TABLE_NAME:
        pytest.skip(f"No deletes for {table_name}.")
    ...
    result = fetch(
        connection, f"SELECT {table_name}_id FROM view_{table_name}_aktuell LIMIT 1;"
    )
    id_ = result[0][0]

    result_pre = fetch(
        connection,
        f"SELECT gueltig_seit, ist_aktiv FROM tab_{table_name} WHERE {table_name}_id = {id_} ORDER BY gueltig_seit DESC;",
    )
