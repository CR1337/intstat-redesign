...
def test_fail_insert(database_connection, table_definition):
    ...
    print(column_values)
    fk_values = [
        str(get_random_value(quote_text=True)) for _ in table_definition["foreign_keys"]
    ]
        ...
        run(
            ...
            "".join(
                [
                    ...
                    (
                        ","
                        if len(table_definition["foreign_keys"])
                        and len(table_definition["columns"])
                        else ""
                    ),
                    ",".join(
                        f"{fk['name']}_id" for fk in table_definition["foreign_keys"]
                    ),
                    ") VALUES (",
                    ",".join(column_values),
                    (
    ...
    assert (
        "Einfuegen (INSERT) ist nur von der entsprechenden PROCEDURE aus erlaubt!"
